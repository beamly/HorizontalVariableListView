...
public abstract class AbsHListView extends AdapterView<ListAdapter> implements ViewTreeObserver.OnGlobalLayoutListener, ViewTreeObserver.OnTouchModeChangeListener {
	...
	public void setAdapter(ListAdapter adapter) {
		if (adapter != null) {
			...
			if (mChoiceMode != ListView.CHOICE_MODE_NONE && mAdapterHasStableIds && mCheckedIdStates == null) {
	...
	public boolean performItemClick(View view, int position, long id) {
		...
		if (mChoiceMode != ListView.CHOICE_MODE_NONE) {
			...
			if (mChoiceMode == ListView.CHOICE_MODE_MULTIPLE || (android.os.Build.VERSION.SDK_INT >= 11 && mChoiceMode == ListView.CHOICE_MODE_MULTIPLE_MODAL && mChoiceActionMode != null)) {
	...
	static class SavedState extends BaseSavedState {
		...
		public String toString() {
			return "AbsListView.SavedState{" + Integer.toHexString(System.identityHashCode(this)) + " selectedId=" + selectedId + " firstId=" + firstId + " viewLeft=" + viewLeft + " position="
					+ position + " width=" + width + " filter=" + filter + " checkState=" + checkState + "}";
		}
	...
	protected float getRightFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			final float fadeLength = (float) getHorizontalFadingEdgeLength();
			return right > width - getPaddingRight() ? (float) (right - width + getPaddingRight()) / fadeLength : fadeEdge;
		}
	}
				...
				if (childCount > 0) {
					...
					if (mFirstPosition == 0 && firstChildLeft >= contentLeft && mFirstPosition + childCount < mItemCount && lastChildRight <= getWidth() - contentRight) {
						...
						if (Math.abs(initialVelocity) > mMinimumVelocity
								&& !((mFirstPosition == 0 && firstChildLeft == contentLeft - mOverscrollDistance) || (mFirstPosition + childCount == mItemCount && lastChildRight == contentRight
										+ mOverscrollDistance))) {
							if (mFlingRunnable == null) {
